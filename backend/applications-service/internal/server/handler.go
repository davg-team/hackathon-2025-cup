...
func (r *ApplicationRouter) GetApplications(ctx *gin.Context) {
	...
	applications, err := r.service.Applications(ctx, applicationStatus, teamID, eventID, userID, dateFilter, fspID)
	if err != nil {
		HandleError(ctx, err)
		return
	}
...
func (r *ApplicationRouter) UpdateApplicationStatus(ctx *gin.Context) {
	...
	if !slices.Contains(payload.Roles, "fsp_staff") && !slices.Contains(payload.Roles, "fsp_region_staff") && !slices.Contains(payload.Roles, "fsp_region_head") && !slices.Contains(payload.Roles, "root") {
